...
public:
  ...
  /**
   * Startup function for the webserver.
   *
  ...
  /**
   * Function is called after the startup and creation of the listening sockets.
   *
...
private:
  /**
   * Function for the single poll call within the webserver.
   *
   * This function contains the single poll call used within the I/O
   * multiplexing webserver. It creates a local std::vector<pollfd_t> from the
   * poll_list member. The vector can be used as input for the poll() function
   * using the &vector[0] syntax. After polling, the revents specified in the
   * pollfd of the vector are then copied back to the poll_list
   *
  ...
  /**
   * Function to handle the specified events for each socket.
   *
   * The function iterates over the poll_list and calls the ISocket::handle()
   * function which returns the ISocket::status and therefore enables to
   * webserver to check if a socket needs to be closed. At the end, the revents
   * of the poll list are being reset.
   */
  void event_handler_();

  /**
   * Function for creating a listening socket during startup.
   *
   * This function creates a listening socket with respect to the specified
  ...
  /**
   * Const reference to a HttpBlock from the configuration file.
   *
  ...
  /**
   * Private member function of the Server.
   *
